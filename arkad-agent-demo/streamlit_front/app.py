...
# )
db_connection_string = f"sqlite:///{Path(__file__).parent / 'StockData.db'}"
if "agent_executor" not in st.session_state:
    ...
    st.session_state["sql_llm"] = ChatOpenAI(
        temperature=0,
        model="gpt-4-0125-preview",
        streaming=True,
        model_kwargs={"seed": 123},
    )
    st.session_state["code_llm"] = ChatOpenAI(
        temperature=0,
        model="gpt-4-0125-preview",
        streaming=True,
        model_kwargs={"seed": 123},
        ...
        if isinstance(msg, HumanMessage):
            ...
            for plot_url in plot_urls:
                st.components.v1.iframe(
                    plot_url, width=1500, height=600, scrolling=True
                )
...
def handle_response(user_input: str):
    ...
    try:
        ...
        traceback.print_exc()
        response = {
            "output": "There was an error while processing your request :( Please, try with new one."
        }
    additional_kwargs = {}
    try:
        ...
        if artifacts_paths:
            ...
            for artifact_path in artifacts_paths:
                ...
                logger.info(
                    f"Attempting to display HTML file: {artifact} at URL: {plot_url}"
                )
    ...
    st.session_state.messages.append(
        AIMessage(content=response_output, additional_kwargs=additional_kwargs)
    )
